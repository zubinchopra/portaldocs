...
* [Feature flags](#feature-flags)
  ...
  These flags connect the extension to testing features that are within the Azure API. The Shell features are maintained by the Azure Portal team, and developers can also create and maintain their own flags.
...
**extloadtimeout**:  Extension loading timeout (in seconds???).  Can contain any characters in the ranges 0-9. 
...
**relex**:  Comma-delimited list of extensions to load in Relex. **NOTE**: `*` will load all extensions in Relex.
...
**showpreviewtags**: Hides the preview ribbon on the startboard and shows labels in blade headers when extensions are marked as in preview   **NOTE**: Does not currently hide the preview ribbon.
...
**webworker**: `=true,microsoft_azure_compute enables webworkers for all extensions who've explicitly been set as supporting webworkers, and forces microsoft_azure_compute into a web worker. Useful for testing extensions manually or through CI before flipping the switch.
